...
CRegexTestDlg::CRegexTestDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_REGEXTEST_DIALOG, pParent)
{
...
BEGIN_MESSAGE_MAP(CRegexTestDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CRegexTestDlg::OnBnClickedButton1)
END_MESSAGE_MAP()


// CRegexTestDlg 메시지 처리기
...
void CRegexTestDlg::OnBnClickedButton1() // 정규식
{
	...
	SetDlgItemText(IDC_EDIT_REGEX, strResult); // 정규식 
	SetDlgItemText(IDC_EDIT_EXECUTE, strResult); // 실행 성공 여부 : 미사용
