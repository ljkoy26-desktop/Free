...
CServerSocketMFCDlg::CServerSocketMFCDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_SERVERSOCKETMFC_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CServerSocketMFCDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CServerSocketMFCDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_START, &CServerSocketMFCDlg::OnBnClickedButtonStart)
END_MESSAGE_MAP()


// CServerSocketMFCDlg 메시지 처리기

BOOL CServerSocketMFCDlg::OnInitDialog()
{
...
void CServerSocketMFCDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CServerSocketMFCDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CServerSocketMFCDlg::OnQueryDragIcon()
{
...
void CServerSocketMFCDlg::ProcessAccept()
{
...
void CServerSocketMFCDlg::OnBnClickedButtonStart()
{
	UINT nPort = 9000;
